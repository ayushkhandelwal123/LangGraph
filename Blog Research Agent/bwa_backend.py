...
parser1 = PydanticOutputParser(pydantic_object=RouterDecision)
evidence_parser = PydanticOutputParser(pydantic_object=EvidencePack)
plan_parser = PydanticOutputParser(pydantic_object=Plan)
image_plan_parser = PydanticOutputParser(pydantic_object=GlobalImagePlan)
...
def router_node(state: State) -> dict:
    ...
    # decision = parser1.parse(model.invoke(
    #     [
    ...
    decision = parser1.parse(response.content)
...
RESEARCH_SYSTEM = f"""You are a research synthesizer.
...
def research_node(state: State) -> dict:
    ...
    print(response)
    pack = evidence_parser.parse(response)
...
{image_plan_parser.get_format_instructions()}
"""
...
def decide_images(state: State) -> dict:
    ...
    image_plan = image_plan_parser.parse(image_plan)
